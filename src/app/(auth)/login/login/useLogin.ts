...
const useLogin = () => {
    ...
    const { logo, logo_width, logo_height } = domainDetails?.data?.domain_detail || {};
    ...
    const { isValid, errors, handleSubmit, values, touched, setFieldValue, handleBlur, validateField } = useFormik({
        initialValues: {
            email: '',
            password: ''
        },
    ...
    const handleLoginApi = useCallback(
        ...
        [dispatch, handleToast, history, isLoading, setToken, updateSocketOnLogin, user]
    );
    ...
    const passwordFormData = useMemo(() => ({
        ...
        position: 'end',
        size: 'large'
    }), [values.password, handleChange, handleBlur, touched.password, errors.password]);
